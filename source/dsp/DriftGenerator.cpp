...
float DriftGenerator::process(float depth) noexcept {
    ...
    const float blockTime = static_cast<float>(samplesPerBlock_) / static_cast<float>(sampleRate_);
    ...
    const float lfoSum = std::sin(twoPi * phase0_) + 0.5f * std::sin(twoPi * phase1_) +
