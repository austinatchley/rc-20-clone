    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        for (int ch = 0; ch < osNumChannels; ++ch)
        {
            ...
            for (int k = 0; k < factor; ++k)
            {
                ...
                switch (mode)
                {
                    case Mode::Tape:       data[idx] = shapeTape      (data[idx], drive); break;
                    case Mode::Tube:       data[idx] = shapeTube      (data[idx], drive); break;
                    case Mode::Transistor: data[idx] = shapeTransistor(data[idx], drive); break;
                    default: break;
                }
            }
        }
